...
class QuickChoiceFormInput : FrameLayout {
    ...
    private fun initView() {
        inflate(context, R.layout.input_quick_choice, this)
    ...
    private fun loadValues() {
        ...
        val quick1Pref = prefs.getString("${tag}_quick1", "")
        val quick2Pref = prefs.getString("${tag}_quick2", "")
        val quick3Pref = prefs.getString("${tag}_quick3", "")
        val quick4Pref = prefs.getString("${tag}_quick4", "")
        val quick5Pref = prefs.getString("${tag}_quick5", "")
        val quick6Pref = prefs.getString("${tag}_quick6", "")
    ...
    private fun setValue(resourceId: Int, nomenclatureItem: NomenclatureItem?) {
        ...
        when (resourceId) {
            R.id.quick_1 -> prefs.edit()
                .putString("${tag}_quick1", nomenclatureItem?.nomenclature?.label?.labelId).apply()
            R.id.quick_2 -> prefs.edit()
                .putString("${tag}_quick2", nomenclatureItem?.nomenclature?.label?.labelId).apply()
            R.id.quick_3 -> prefs.edit()
                .putString("${tag}_quick3", nomenclatureItem?.nomenclature?.label?.labelId).apply()
            R.id.quick_4 -> prefs.edit()
                .putString("${tag}_quick4", nomenclatureItem?.nomenclature?.label?.labelId).apply()
            R.id.quick_5 -> prefs.edit()
                .putString("${tag}_quick5", nomenclatureItem?.nomenclature?.label?.labelId).apply()
            R.id.quick_6 -> prefs.edit()
                .putString("${tag}_quick6", nomenclatureItem?.nomenclature?.label?.labelId).apply()
        }
        updateButtons()
    }
